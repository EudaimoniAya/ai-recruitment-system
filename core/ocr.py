...
class PaddleOcr:
    def __init__(self):
        ...
        self.access_token = settings.paddle_api_key
        ...
        if file.startswith("http"):
            ...
            async with httpx.AsyncClient() as client:
                job_resp = await client.post(
                    self.job_url, json=payload, headers=self.headers
                )
        else:
        ...
        while True:
            async with httpx.AsyncClient() as client:
                ...
                if state == "pending":
                    ...
                    try:
                        total_pages = job_result_response.json()["data"][
                            "extractProgress"
                        ]["totalPages"]
                    ...
                    jsonl_url = job_result_response.json()["data"]["resultUrl"][
                        "jsonUrl"
                    ]
                    return jsonl_url
                elif state == "failed":
