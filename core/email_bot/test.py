...
async def poll_and_reply(bot: EmailBot, state: dict):
    try:
        ...
        for mail in new_emails:
            ...
            state["last_uid"] = (
                max(state["last_uid"], int(mail.uid))
                if str(mail.uid).isdigit()
                else state["last_uid"]
            )

        logger.info(
            "processed {} new emails, last_uid now {}",
            len(new_emails),
            state["last_uid"],
        )
...
async def main():
    ...
    async with EmailBot(settings) as bot:
        ...
        scheduler.add_job(
            poll_and_reply, "interval", seconds=15, args=[bot, state], max_instances=1
        )
        scheduler.start()
